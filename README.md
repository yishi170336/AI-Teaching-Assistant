...
1. PDF 页面渲染或图片标准化后，`qwen3-vl-flash` 先并发分析每 4 页一组的可读联系表，把页面分为题目、答案、混合、无关和疑似页；疑似页及候选页的相邻页会保守纳入，纯理论、目录等页面不进入高清提取，同一原文件重试时会复用粗筛缓存。
2. PDF-Extract-Kit 为候选页给出版面区域，`qwen3-vl-flash` 仅对候选页高清提取题型、题目标识、共同题干/分层小问/选项/插图/答案 bbox、参考答案和评分点；只有混合页、低置信度页或首次结果不完整时才二次校对，跨页题干合并时会去除重复复述。
