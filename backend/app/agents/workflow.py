...
class CircuitTutorEngine:
    ...
    async def _compose_answer_prompt(self, state: AgentState) -> AgentState:
        ...
        question_knowledge_overview = question_knowledge and any(
            marker in re.sub(r"\s+", "", str(state.get("message", "")))
            for marker in (
                "有什么知识点", "有哪些知识点", "包含什么知识", "包含哪些知识",
                "里面的知识点", "涉及什么知识", "涉及哪些知识", "用到什么知识",
                "用到哪些知识", "知识点总览", "知识点总结", "考点", "考什么",
                "考查什么", "考察什么", "本题重点", "这道题重点", "这题重点",
                "重点和难点", "主要内容",
            )
        )
        if question_knowledge:
            if question_knowledge_overview:
                system = (
                    "你是题库题目知识点分析助教。本轮分析对象只能是服务器绑定的当前原题，题库参考答案用于确认"
                    "这道题实际采用的模型、公式、判断规则和结论边界。学生本轮要的是题目知识点总览：先用一句话"
                    "说明题目真正考查什么，再列出3—6个知识点；每个知识点都必须指出它对应题干、题图或参考答案"
                    "中的哪一项，以及为什么对本题重要。可以根据对应关系选择简短表格或列表，不要机械套固定版式。"
                ...
                system = (
                    "你是题库题目的概念讲解助教。学生已经从当前题目的知识点中点名了一个模型、概念、公式、"
                    "符号或判断规则，本轮只围绕这个被点名的对象继续讲解。禁止再次输出整道题的知识点总表，"
                    "禁止复述上一轮的栏目、表格和大段原话，也不要用‘知识点—对应内容—为什么重要’模板。"
                    "先用一两句直观说明回答‘它是什么、用来解决什么问题’，再根据学生实际问法自然展开其组成或"
                    "符号、关键关系、成立条件，以及它在当前题目中承担的作用；不相关的部分可以省略，不强制凑齐栏目。"
                    "优先使用短段落和必要的项目符号，除非确有多项一一对应关系，否则不要使用表格。语气应像教师"
                    "接着上一轮往下讲，允许使用‘可以把它理解为……’等自然过渡，但不要写空泛开场。"
                    "当前原题和参考答案只用于限定语境，不得借机完整解题、代入数值或泄露最终答案，也不得改变"
                    "题图拓扑、器件极性、符号含义和参考答案边界。正文通常控制在350—650字。"
                )
        ...
        system += (
            " 输出必须保证 Markdown 与 LaTeX 可直接渲染：Markdown 表格单元格只能放普通文字或简短的"
            "行内公式 $...$，不得在表格中放 $$...$$ 块公式；较长公式必须移到表格外单独成段。"
            "绝对值和范数使用 \\lvert...\\rvert，不要在表格公式中使用裸竖线字符。"
            "所有 $ 或 $$ 定界符、花括号和 LaTeX 环境必须成对闭合。"
        )
        if state.get("scene") == "image_answer":
            system += (
        ...
        if question_knowledge:
            ...
            user += (
                ...
                + (
                    "题目知识点总览；可以归纳多个知识点。"
                    if question_knowledge_overview
                    else "点名概念聚焦讲解；只回答学生点名的对象，不得重复知识点总表。"
                )
                + "\n\n本轮必须分析的当前原题：\n"
